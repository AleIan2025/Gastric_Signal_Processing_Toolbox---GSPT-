...
% Utilizziamo i valori scelti dall'utente
...
if nCh > 1
    ...
    for sc = 1:n_scenarios
        ...
        % Subset data arrays
        sub_fill_s  = fill_s(mask);
        sub_r       = r(mask);
        sub_tp      = tp(mask);
        sub_ief     = ief(mask);
        sub_amp     = amp_curve(mask);
        sub_if      = if_curve(mask);
        sub_ph      = ph_curve(mask);
        sub_ridge   = ridge_curve(mask);
        ...
        if any(~sub_r_tot)
            ...
            % Total correlations
            try TC_ch    = total_corr_general(build_matrix(sub_fill_s), 2); catch, TC_ch = NaN; end
            try TC_en    = total_corr_general(build_matrix(sub_tp), 2);     catch, TC_en = NaN; end
            try TC_ief   = total_corr_general(build_matrix(sub_ief), 2);    catch, TC_ief = NaN; end
            try TC_eamp  = total_corr_general(build_matrix(sub_amp), 2);    catch, TC_eamp = NaN; end
            try TC_efreq = total_corr_general(build_matrix(sub_if), 2);     catch, TC_efreq = NaN; end
            try TC_ephi  = total_corr_general(build_matrix(sub_ph), 2);     catch, TC_ephi = NaN; end
            try TC_ridge = total_corr_general(build_matrix(sub_ridge), 2);  catch, TC_ridge = NaN; end
            ...
            min_len = min(cellfun(@length, sub_fill_s)); data_matrix = data_matrix(1:min_len, :);
            
            try [MPerm, ~] = MvPermEn(data_matrix, 'm', eDim, 'tau', eLag); catch, MPerm = NaN; end
            try [MDisp, ~] = MvDispEn(data_matrix, 'm', eDim, 'tau', eLag); catch, MDisp = NaN; end
            
            metrics = [TC_ch, TC_en, TC_ief, TC_eamp, TC_efreq, TC_ephi, TC_ridge, MPerm, MDisp];
        else
            metrics = NaN(1, 9);
        end
  ...
  %% 10. PLOT FINAL RESULTS (SPIDER PLOT)
    ...
    disp('Global Total correlations (All Channels):')
...
for ch = 1:nCh
    ...
    % 1. Valutazione Monte Carlo (Simes)
    if all_reliable(ch) 
    ...
    % 2. Valutazioni GSPT (SD Greedy Dropping)
