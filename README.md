...
You can run GSPT using the script `EGG_main_multichannel_v7_with_choice`. Before running it, ensure you update the lines of code highlighted in the red boxes in the figure below to suit your needs.
...
If the **Forced** option is selected, artifact localization and removal are performed by default. The **Conditional** mode protects against an excessive number of false positives at the expense of losing some true positives. Conversely, the **Forced** option is more aggressive, leading to a higher number of true positives at the cost of more false positives. The choice between the two is left to the user's specific needs.
...
At rest, healthy gastric slow waves propagate coherently across the stomach, meaning the Dominant Frequency (DF) should remain spatially consistent across all recording channels. GSPT leverages this property to perform automated quality control by evaluating the maximum DF range $\Delta_{max}$ (the difference between the highest and lowest channel DFs). Because there is no theoretical first principle to establish a single universal cutoff, GSPT implements three candidate preset thresholds: strict (0.005 Hz), medium (0.010 Hz), and loose (0.020 Hz), which can also be customized by the user. Recordings with an inter-channel DF range below the selected threshold are retained as reliable, while those exceeding it are flagged as corrupted. Assuming the measured DFs reflect a single underlying biological oscillator perturbed by additive Gaussian noise, this range threshold maps to the channel-wise sample standard deviation $s$ via the following relationship:

$$\Delta_{max}= s \frac{d_2(n)}{c_4(n)},$$
