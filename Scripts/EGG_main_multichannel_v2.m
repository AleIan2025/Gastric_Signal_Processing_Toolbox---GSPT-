...
output_folder = 'C:\Users\FSL\Documents\MATLAB\Prova_stampa_pipeline';

% ⚠️ Update the path below to match your local FieldTrip installation.
fieldtrip_path=addpath('C:\Users\FSL\Documents\MATLAB\fieldtrip-20231113');
...
cfg.dataset='C:\Users\aless\Downloads\Example_Artifact.eeg';

cfg.dataset = 'C:\Users\FSL\Documents\MATLAB\prova_franz_1_baseline.eeg';

cfg.dataset = 'C:\Users\FSL\Documents\MATLAB\prova_rita_1_10min.eeg';
...
for ch = 1:nCh
    ...
    [~, norm_pow{ch}, parameters{ch}, efreq{ch}, eamp{ch}, ephi{ch}, ...
     ...
     pipeline_single_channel_v11(x(ch,:), t, fs, freq_range, ch);
end
...
if any(~r_tot)
  ...
  %% 10. PLOT FINAL RESULTS (SPIDER PLOT)
    ...
    % Etichette per il plot (leggermente accorciate per estetica)
    ...
    % Disegno Manuale Etichette e Guide
    hold on;
    ...
    % Display computed values nel Command Window
...
n_rows   = nCh;
% One extra column for is_noise
table_cell = cell(n_rows+1, n_fields+2);
% Header
table_cell{1,1} = 'Channel';
for f = 1:n_fields
    table_cell{1,1+f} = field_names{f};
end
table_cell{1,n_fields+2} = 'is_noise'; 
% Fill each row
for ch = 1:nCh
    ...
    if isfield(parameters_results{ch}, 'spec_skewness') && isfield(parameters_results{ch}, 'spec_sparsity')
        ...
        if is_globally_significant
            table_cell{ch+1,n_fields+2} = 'no'; % not noise
        else
            table_cell{ch+1,n_fields+2} = 'yes'; % noisy
        end
    else
        table_cell{ch+1,n_fields+2} = 'unknown';
    end
    % ============================
end
% Write Excel file
