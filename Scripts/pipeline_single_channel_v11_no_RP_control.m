function [time_pow, norm_pow, parameters, efreq, eamp, ephi, r1, tp, ief, amp_curve, ph_curve, if_curve, filled_signal, ridge_curve, parameters_results] = pipeline_single_channel_v11_no_RP_control(x, t, fs, freq_faslt, channel_id)
...
%   [Multiple outputs related to signal processing results]
    ...
    sgtitle(sprintf('Raw FASLT for channel %d', channel_id));
    
        % In this version of the single channel processing each signal is
        % always signed as potentially Contaminated by artifacts
        [parameters, efreq, eamp, ephi, r1, tp, ief, amp_curve, ph_curve, if_curve, filled_signal, ridge_curve, parameters_results, norm_pow, time_pow] = ...
            process_contaminated_channel(x, t, fs, freq_faslt, channel_id);    
    toc;
end

%% Helper functions =======================================================

function [parameters, efreq, eamp, ephi, r1, tp, ief, amp_curve, ph_curve, if_curve, filled_signal, ridge_curve, parameters_results, norm_pow, time_pow] = ...
    process_contaminated_channel(x, t, fs, freq_faslt, channel_id)
    ...
    if any(r1 == 0) 
        %% Parameter extraction from reconstructed signal
        ...
        create_spider_plot(parameters, channel_id, is_globally_significant, r1);
    else
...
function create_spider_plot(parameters, channel_id, is_reliable, r1)
% CREATE_SPIDER_PLOT
    %% 1. Check preliminary
    ...
    config_db = {
        ...
        'TFR GradEn',          [0, 5],         'linear', 2, '%.2f';  
        'Rel Dom GradEn',      [0, 5],         'linear', 2, '%.2f';  
        
        % --- TIME DOMAIN ENTROPIES ---
        'Sig DispEn',      [0, 5],         'linear', 1, '%.1f';  
        'Sig SampEn',      [0, 5],         'linear', 1, '%.1f';  
        'IE DispEn',       [0, 5],         'linear', 1, '%.1f';  
    ...
    full_subtitle = sprintf('%s | Fraction of excluded signal: %.2f', subtitle_str, mean(r1));
