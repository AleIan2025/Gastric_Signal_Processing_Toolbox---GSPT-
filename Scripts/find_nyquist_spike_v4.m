function [index, x_new] = find_nyquist_spike_v4(x, t, fs, delay)
% FIND_NYQUIST_SPIKE_V3 
% Detects and corrects spikes or jumps in a signal.
% Handles:
%   - Persistent jumps: signal shifts and stays at a new level
%   - Short transient spikes
% Includes robust handling for cases where the spike occurs at the beginning
% of the signal or when too few samples are available for local statistics.
%
% INPUTS:
%   x     - input signal
%   t     - time vector
...
if ~isempty(index)
    ...
    for i = 1:length(index)
        ...
        %% ----------------------------------------------------------
        %  (A) ROBUST ESTIMATION OF LOCAL MEAN AND STD
        % ----------------------------------------------------------
        ...
        if duration >= min_duration_for_jump
            % Persistent jump: shift subsequent samples to maintain trend
            x_new(index(i)) = local_mean;
            x_new(index(i)+1:end) = ...
                x_new(index(i)+1:end) - x(index(i)+1:end) + local_mean;
        else
